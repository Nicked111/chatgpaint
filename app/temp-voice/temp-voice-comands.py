...
UPDATE_SUCCESS_MESSAGE: Final[str] = "Update successful!"
SLOW_DOWN_MESSAGE: Final[str] = "You are doing this too fast!"
...
class TempVoiceInterface(discord.ui.Button):  # Button for the Temp Voice Interface
    def __init__(self, cmd_id: int):  # cmdId is the command id
        super().__init__(  # create the button
            label=tempVoiceCmdIds[cmd_id],  # set the label to the command name
            style=discord.ButtonStyle.secondary,  # set the style to secondary
            custom_id=str(cmd_id),  # set the custom id to the command id
        )

    async def callback(self, interaction: discord.Interaction):  # when the button is clicked
        cmd_id = int(self.custom_id)  # get the command id
        ...
        if user_channel.category_id != joinToCreateParent: return await interaction.response.send_message(
            "You must be in a temporary voice channel to use this command!", ephemeral=True)

        no_channel_owner_text = "You must be the channel owner to use this command!"
        if not await member_is_channel_owner(user_channel.id,
                                             interaction.user.id):  # if the member is not the channel owner
            return await interaction.response.send_message(no_channel_owner_text,
        ...
        match cmd_id:
            ...
            case 1:
                await interaction.response.send_modal(
                    LimitChannel(title="Set Userlimit"))  # send the limit channel modal
            case 2:
...
class TempVoiceCog(commands.Cog):
    ...
    async def tempVoiceInterface(self, ctx: discord.ApplicationContext):  # when the command is called
        ...
        for cmd_id in tempVoiceCmdIds:  # for every command id
            view.add_item(TempVoiceInterface(cmd_id))  # add the button to the view
    ...
    async def callback(self, interaction: discord.Interaction):  # when the modal is submitted
        ...
        if channel is not None:  # if the channel exists
            try:
                ...
                embed.title = UPDATE_SUCCESS_MESSAGE  # set the title
...
class ClaimChannel(CooldownSetter):
    ...
    async def callback(self, interaction: discord.Interaction):  # when the command is called
        if self.i_did_smth_too_fast(interaction.user):
            await interaction.response.send_message(SLOW_DOWN_MESSAGE, ephemeral=True)
        else:
            user_channel = interaction.user.voice.channel
            if await member_is_channel_owner(user_channel.id, interaction.user.id):  # if the member is the channel owner
            ...
            for member in interaction.user.voice.channel.members:  # for every member in the channel
                if member.id == await TempVoiceBackend().get_owner_id(
                        user_channel.id):  # if the member id is the same as the member id
            ...
            await TempVoiceBackend().change_channel_owner_id(user_channel.id,
...
class LockChannel(CooldownSetter):
    ...
    async def callback(self, interaction: discord.Interaction):  # when the command is called
        if self.i_did_smth_too_fast(interaction.user):
            return await interaction.response.send_message(SLOW_DOWN_MESSAGE, ephemeral=True)
...
async def member_is_channel_owner(channel_id, member_id) -> bool:
